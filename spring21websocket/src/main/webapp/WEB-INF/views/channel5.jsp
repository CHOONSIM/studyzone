...
<script>
	$(function(){
		...
		// 시작하자마자 연결
			...
			// - 지금 현재 서버의 대기실에 위치하고 있으므로 방번호를 알려줘서 이동처리
			window.socket.onopen = function(){
				// 파라미터 중에서 room이라는 항목을 읽어서 첨부하여 전송
				// const data = {type : 2, room : "${param.room}"};	// jsp에서도 쓸 수 있음(비추)
				
				const room = new URLSearchParams(location.search).get("room");		//front에서도 파라미터를 읽으 수 있음
				const data = {type : 2, room : room};	
				window.socket.send(JSON.stringify(data));	//window.socket.send(채널이동메세지);
