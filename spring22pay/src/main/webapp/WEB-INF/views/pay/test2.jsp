...
<script>
	$(function(){
		//기존 form을 전송하는 대신 체크된 항목들만 새로운 form을 복사하여 전달
		$(".purchase-form").submit(function(e){
			e.preventDefault();		//기본이벤트 차단
			
			//신규 폼을 기존 폼과 동일하게 생성
			const newForm = $("<form>").attr("action",$(this).attr("action")).attr("method","post");
...
// 				console.log(itemNo, qty);
				...
				$("<input>").attr("type","hidden").attr("name","data["+index+"].itemNo").val(itemNo).appendTo(newForm);
				$("<input>").attr("type","hidden").attr("name","data["+index+"].qty").val(qty).appendTo(newForm);
			});
